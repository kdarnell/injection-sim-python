...
class FlashController(object):
    ...
    def main_handler(self, compobjs, z, T, P,
        ...
        if initialize or not self.completed:
            ...
            alpha_new, theta_new, k_iter = self.find_alphatheta_min(z, alpha_0,
                                                            theta_0, K_0,
            ...
            alpha_new = alpha_calc.copy()
            theta_new = theta_calc.copy()
            K_new = K_calc.copy()
        ...
        while ((error > TOL) and (itercount < iterlim)) or (itercount < 3):
            # Perform newton iteration to update alpha and theta at
            # a fixed x and K
            self.iter_output = {}
            alpha_new, theta_new, k_iter = self.find_alphatheta_min(z, alpha_old,
                                                            theta_old, K_new,
                                                            monitor_calc=monitor_calc)
...
#        x = np.concatenate((alpha0, theta0))
        ...
        new_values = [alpha_new, theta_new, k]
        return new_values
