...
class model(object):
    ...
    def set_temperature(self, T_L, T_R):
        if self.T_grad is not None:
            self.temperature = np.linspace(T_L, T_R, len(self.xc))
        else:
            self.temperature = np.zeros_like(self.xc)
            ...
            for filename in file_list:
                ...
                if P == P_get:
                    ...
                    lookup_dict[T] = {'fn': filename,
                                      ...
                                      'flash': l_table.flash,
                                      'lt': l_table}
            self.lookup_tbl = lookup_dict

    def flash_wgrad(self, z, T_q):
        lookup_sorted = sorted(self.lookup_tbl.keys())
        ind_q = np.interp(T_q, lookup_sorted, range(len( self.lookup_tbl)))
        ind_set = [int(np.floor(ind_q)), int(np.ceil(ind_q))]
        t_interp_results = [ self.lookup_tbl[lookup_sorted[ii]]['flash'](z) for ii in ind_set]
        if ((t_interp_results[0][0] == t_interp_results[1][0])
            and (ind_set[0] != ind_set[1])
            and (t_interp_results[1][1][0] != t_interp_results[0][1][0]).all()):
            comp = (t_interp_results[0][1][0]
                    + (t_interp_results[1][1][0] - t_interp_results[0][1][0])
                       / (ind_set[1] - ind_set[0])*(ind_q - ind_set[0]))
            ...
            output =  self.lookup_tbl[lookup_sorted[int(np.round(ind_q))]]['flash'](z)
        return output

    def set_model(self, zL, zR, rho, vL=1, mu=None, S_c=None, Cor_exp=None, T_grad=None, T_L=None, T_R=None):
        if mu is None:
            mu = np.ones_like(rho)
    ...
    def density_closure(self, vd, G, H, T_info=None):
        ...
        if ((z_guess == 0).all(-1)).any() or ((np.isnan(z_guess)).all(-1)).any():
            pdb.set_trace()
        x_mat_guess, alpha_mat_guess = self.flash(z_guess, T_info)
    ...
    def jac_density_closure(self, vd, G, H, eps=1e-6, T_info=None):
        Residue = self.density_closure(vd, G, H, T_info=T_info)
        Jac = np.zeros((len(vd), len(vd)))
        for ii in range(len(vd)):
            eps_mat = np.zeros(len(vd))
            eps_mat[ii] = eps
            Jac[:, ii] = (self.density_closure(vd + eps_mat, G, H, T_info=T_info)
                          - self.density_closure(vd - eps_mat, G, H, T_info=T_info)) / (2*eps)
        return Jac

    def jac_res(self, vd, G, H, eps=1e-6, T_info=None):
        Residue = self.density_closure(vd, G, H, T_info=T_info)
        Jac = np.zeros((len(vd), len(vd)))
        for ii in range(len(vd)):
            eps_mat = np.zeros(len(vd))
    ...
    def newton_solve(self, vd, G, H, eps=1e-10, T_info=None):
        error, J = self.jac_res(vd, G, H, T_info=T_info)
        if np.isnan(vd).any():
            pdb.set_trace()
        count = 0
        # pdb.set_trace()
    ...
    def flash(self, z_mat, T_info=None):
        ...
        if len(z_mat.shape) > 1:
            ...
            if self.T_grad is None:
                ...
                result = self.flash_wgrad(z_mat, T_info)
    ...
    def run_model(self, t_end=1.0, cfl=0.01, save_int=10, vd_max=5):
        ...
        if self.set_flag:
            ...
            if self.T_grad is None:
                ...
                z_bndry = np.vstack([self.zL, self.zR])
                T_bndry = np.vstack([self.temperature[0], self.temperature[-1]])
                x_bndry, alpha_bndry = self.flash(z_bndry, T_info=T_bndry)
            ...
            for tstep in range(self.tsteps):
                ...
                output = self.newton_solve(vd_guess,
                                           G[:self.front, :],
                                           H[:self.front, :],
                                           T_info=self.temperature[:self.front])
                ...
                z_new[z_new < 1e-20] = 0
                x_mat_new, alpha_mat_new = self.flash(z_new, T_info=self.temperature[1:self.front])
